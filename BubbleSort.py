...
# Bubble Sort
def bubbleSort(indexImgArr, imgStoragearr, originalImgarr):
	...
	for i in range(len(indexImgArr)): 
		...
		for j in range(0,len(indexImgArr)-i-1):

			if indexImgArr[j] > indexImgArr[j+1]:

				indexImgArr[j],indexImgArr[j+1] = indexImgArr[j+1], indexImgArr[j]
...
def main():
	...
	root = Tk()
	root.title("Bubble Sort Visualizer -- Navanil Sengupta")
	...
	sortImgArr = bubbleSort(indexA, imgArr, A)
